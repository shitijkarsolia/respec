...
A ~45s silent, captioned product demo of [Respec](https://respec-ai.vercel.app), authored
as **video-as-code** with [HeyGen HyperFrames](https://github.com/heygen-com/hyperframes)
(HTML + GSAP → deterministic 1920×1080 / 30fps MP4).

## How it's made

```
capture.mjs   Playwright drives the REAL app and records per-scene clips
   │            (dark mode, injected cursor + click ripples, tour hidden)
   ▼
convert.sh    trims the lead-in and re-encodes each clip to a seekable mp4
   │
   ▼
index.html    HyperFrames composition: branded intro/outro (GSAP) + the clips
   │            as <video> tracks + animated lower-third captions
   ▼
render        npx hyperframes render → renders/respec-demo.mp4
```

Storyboard: intro → landing → canvas reveal → hover-to-trace → agent flags →
annotate → compile feedback → approve + handoff → outro.
...
# 2. capture the real flow (Playwright comes from ../respec/node_modules)
cd ../demo-video && node capture.mjs

# 3. trim + encode the clips
bash convert.sh
...
`assets/*.mp4` are the captured scene clips the composition plays; `assets/raw/`
